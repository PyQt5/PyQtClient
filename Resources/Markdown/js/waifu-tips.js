...
$(document).on('copy', function () {
    showMessage('你都复制了些什么呀，要记得加上出处哦！', 8000);
});

$(window).resize(function () {
    var pos = $('.waifu').position();
    var otop = window.innerHeight - 250;
    var oleft = window.innerWidth - 250;

    if (pos.top > otop)
        $(".waifu").css('top', otop);
    if (pos.left > oleft)
        $(".waifu").css('left', oleft);
});
...
jQuery(document).ready(function ($) {
    ...
    $(document).on("mouseover", "p a", function () {
        ...
        if (text.startsWith('运行') || text.startsWith('查看') | text.startsWith('预览'))
